...
2. Проверка данных
bash:
docker exec -it bd_snowflake_postgres psql -U postgres -d postgres -c "SELECT COUNT(*) FROM public.mock_data;"

3. Проверка модели снежинка
bash:
docker exec -it bd_snowflake_postgres psql -U postgres -d postgres -c "SELECT COUNT(*) FROM analytics.fact_sales;"